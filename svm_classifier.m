function [test_acc, test_sens, test_spec, model] = svm_classifier(X,y,kernel,kernel_par,C,training,test,standardize)
% standadize ==1 normalizes examples
if nargin < 8
    standardize = 0;
end
    ...
    if standardize == 2
        train_X = X(1:length(training),:); % form the training data
        train_y = y(1:length(training))'; 

        test_X = X(length(training)+1:end,:); % form the testing data
        test_y = y(length(training)+1:end);
    else
...
if strcmp(kernel,'linear')
    if standardize == 1
    model = fitcsvm(train_X, train_y, 'KernelFunction', kernel, 'KernelScale' , kernel_par, 'BoxConstraint', C,'Standardize',true); 
    else
    model = fitcsvm(train_X, train_y, 'KernelFunction', kernel, 'KernelScale' , kernel_par, 'BoxConstraint', C); 
    % train an SVM model with a non-linear kernel and its kernel parameter using the training data
    end
end
...
if strcmp(kernel,'polynomial') 
    model = fitcsvm(train_X, train_y, 'KernelFunction', kernel, 'PolynomialOrder' , kernel_par, 'BoxConstraint', C); 
    % train an SVM model with a non-linear kernel and its kernel parameter using the training data
end
